                                    ...
                                    self.makeMissingAdunitsLists(function () {
                                        self.createMissingAdunits(function () {
                                            self.CreateOrUpdateMediationGroup(function () {
                                                ...
                                                self.finishDialog();
                                                self.sendReports({mode: 0, note: "json"}, [JSON.stringify({message: "Finish", admob: self})], function () {console.log("Sent finish inventory report");});
                                                callback();
                                            });
                                        })
                                    })
...
AdmobV2.adunitsScheme = function (app) {
    ...
    scheme.push({
        app: app.localApp[1],
        name: AdmobV2.adunitName(app, "interstitial", "text"),
        adType: 1,
        formats: [0]
    });
    scheme.push({
        app: app.localApp[1],
        name: AdmobV2.adunitName(app, "banner", "image"),
        adType: 0,
        formats: [1]
    });
    scheme.push({
        app: app.localApp[1],
        name: AdmobV2.adunitName(app, "banner", "text"),
        adType: 0,
        formats: [0]
    });
    scheme.push({
        app: app.localApp[1],
        name: AdmobV2.adunitName(app, "mrec", "image"),
        adType: 0,
        formats: [1]
    });
    scheme.push({
        app: app.localApp[1],
        name: AdmobV2.adunitName(app, "mrec", "text"),
        adType: 0,
        formats: [0]
    });
    ...
    $.each(OperationSystemMissingSchemeMediationGroup, function (index, value) {
        if (value.length > 0) {
            ...
            value.forEach(function (item) {
                ...
                var type = 0;
                var bidflor = "10000";
                var matchedType = item.replace(/^Appodeal(\/\d+)?\/(banner|interstitial|mrec|rewarded_video)\/(image|text|rewarded)\//, "");
                if (!isNaN(matchedType)) bidflor = (parseFloat(matchedType) * 1000000).toString();
                var adTypeName = AdmobV2.adUnitRegex(item).adType;
